...
def run(tax_repo: Path, dataset_parquet: str, output_dir: Path | None) -> dict:
    """Execute one native C4 batch and compare online with offline accumulation."""
    ...
    with (
        ...
        outputs = inference_zord.forward_with_hooks_step(
            batch=batch, hooks=hooks
        ).outputs
    arrays = prepare_export_arrays(outputs, batch, hooks)

    accumulator = TaxActivationAccumulator(hooks, n_bins=16, seed=0)
    selected_samples = accumulator.update(arrays)
    ...
    with output_context as managed_output:
        ...
        results = {
            ...
            "seed": 0,
            "n_batches": 1,
            "total_samples": int(arrays["input_token"].shape[0]),
            "total_selected_samples": selected_samples,
            **accumulated_results,
        }
        write_results(str(online_root), results)
        if {path.name for path in online_root.iterdir()} != {"results.json"}:
            raise AssertionError("Online mode persisted files other than results.json.")

        with tempfile.TemporaryDirectory(
            prefix="tax_entropy_offline_reference_"
        ) as tmp:
            reference_root = Path(tmp)
            shard_name = write_npz_shard(str(reference_root), 0, arrays)
...
def main() -> None:
    ...
    run(tax_repo, args.dataset_parquet, args.output_dir)
